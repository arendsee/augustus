...
void exon_segmentation::make_gff(dataset &coverage_info,string outputfile,int c,float s,string train_file,int flag_r,string bed_file,string augustus_hints,int chunksize){
	...
	for(i=0;i<segmentation.size();i++){
	  if(segmentation[i].state==EXONP)
	    outfile1<<"chr2L\tsource\t"<<"exon\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"1\t+\t0\tgrp"<<i<<"-"<<segmentation[i].avg_cov<<"\n";
	  else if(segmentation[i].state==EXONM)
	    outfile1<<"chr2L\tsource\t"<<"exon\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"1\t-\t0\tgrp"<<i<<"-"<<segmentation[i].avg_cov<<"\n";
	  else
	    outfile1<<"chr2L\tsource\t"<<"intron\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"1\t-\t0\tgrp"<<i<<"-"<<segmentation[i].avg_cov<<"\n";
	...
	if(J[J_pointer]==i+1){
	  J_pointer++;
	  augustus_hint<<"chr2L\twig2hints\t"<<"diffgene\t"<<(segmentation[i].end+segmentation[i].start)/2<<'\t'<<(segmentation[i+1].end+segmentation[i+1].start)/2<<'\t'<<"-"<<"\t+\t.\tsrc=W;left="<<segmentation[i].start<<".."<<segmentation[i].end<<";right="<<segmentation[i+1].start<<".."<<segmentation[i+1].end<<"\n";
	}
	if(segmentation[i].state==EXONP){
	  outfile<<"chr2L\tsource\t"<<"exon\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"1\t+\t0\tgrp"<<i<<"-"<<J_pointer<<"-"<<segmentation[i].avg_cov<<"\n";
	  augustus_hint<<"chr2L\twig2hints\t"<<"exonpart\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<segmentation[i].avg_cov<<"\t+\t.\tsrc=W"<<"\n";
	  bed_format<<"chr2L\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"exon-"<<segmentation[i].avg_cov<<"\t"<<segmentation[i].avg_cov<<"\t+\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<"\t0,255,0\n";
	}
	else if(segmentation[i].state==EXONM){
	  outfile<<"chr2L\tsource\t"<<"exon\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"1\t-\t0\tgrp"<<i<<"-"<<J_pointer<<"-"<<segmentation[i].avg_cov<<"\n";
	  augustus_hint<<"chr2L\twig2hints\t"<<"exonpart\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<segmentation[i].avg_cov<<"\t-\t.\tsrc=W"<<"\n";
	  bed_format<<"chr2L\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<'\t'<<"exon-"<<segmentation[i].avg_cov<<"\t"<<segmentation[i].avg_cov<<"\t-\t"<<segmentation[i].start<<'\t'<<segmentation[i].end<<"\t0,0,255\n";
	}
	else
     ...
     */
      ...
      for(i=0;i<J.size();i++){
	bed_format<<"chr2L\t"<<(segmentation[J[i]-1].end+segmentation[J[i]-1].start)/2<<'\t'<<(segmentation[J[i]].end+segmentation[J[i]].start)/2<<'\t'<<"GENE_JUMP\t"<<"500"<<"\t+\t"<<(segmentation[J[i]-1].end+segmentation[J[i]-1].start)/2<<"\t"<<(segmentation[J[i]].end+segmentation[J[i]].start)/2<<"\t255,0,0\n";
      }      
    }
    outfile.close();  
    augustus_hint.close();
